...
export class AIService {
  ...
  static async generateTravelPlan(input: GenerationInput) {
    ...
    const prompt = `
      ...
      1. Determine the relevant local currency based on the Origin place (or Destination if origin is not helpful to determine a currency, e.g., if traveling from India 'INR/₹', from France 'EUR/€', from UK 'GBP/£', from Japan 'JPY/¥', etc.). All cost numbers (activities, budgetEstimate, hotelSuggestions) in your entire response MUST be generated in this currency code/symbol. Output the 3-letter currency code (e.g., USD, INR, EUR) and the symbol (e.g., $, ₹, €).
      2. Draw up a Day-by-Day itinerary that starts from Day 1 and intelligently factors in transit/travel time, travel speed, and departure timings:
         ...
         - For ${input.travelMode}: Compute realistic transit duration between ${input.origin} and ${input.destination}.
           * e.g., Flight is faster but has airport security/boarding time. Train, Bus, or Driving may take many hours (e.g., 8-24 hours).
      ...
      - Estimate realistic numbers in the identified local currency and use its symbol.
    `;
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: prompt,
        config: {
          systemInstruction: "You are a professional world-travel consultant. Generate precise JSON that matches the required responseSchema perfectly.",
